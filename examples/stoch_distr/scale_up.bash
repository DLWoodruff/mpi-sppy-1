...
for size in "${sizes[@]}"; do
    for scens in "${scencnts[@]}"; do
        ...
        echo "^^^^**** mpiexec -np 3 python -u -m mpi4py stoch_distr_admm_cylinders.py --num-stoch-scens  ${scens} --num-admm-subproblems 5 --default-rho 10 --solver-name ${SOLVERNAME} --max-iterations ${ITERS} --scalable --xhatxbar --lagrangian --mnpr $size --ensure-xhat-feas --rel-gap 0.01"
        mpiexec -np 3 python -u -m mpi4py stoch_distr_admm_cylinders.py --num-stoch-scens  ${scens} --num-admm-subproblems 5 --default-rho 10 --solver-name ${SOLVERNAME} --max-iterations ${ITERS} --scalable --xhatxbar --lagrangian --mnpr $size --ensure-xhat-feas --rel-gap 0.01
    done
done
