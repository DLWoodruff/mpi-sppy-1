...
tighten, so when the interval itself matters, control the batch optimality gap
with ``--rel-gap`` in the batch config file (see below) rather than relying on
the outer bound to err in a safe direction.
