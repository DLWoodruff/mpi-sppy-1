...
class Stoch_AdmmWrapper(): #add scenario_tree
    ...
    def release_scenario_models(self, models):
        ...
        Unlike a plain PH run, a stoch-ADMM run builds every wrapped scenario
        at startup whether or not it is about to resume -- the wrapper needs
        the built models to assemble consensus lists, ``varprob_dict``, node
        names and the objective scaling -- so a resume that swaps dilled
        models into ``local_scenarios`` leaves the freshly built ones alive
        here, and the run carries two copies of every scenario for its whole
        life. On large MIPs that is the memory the checkpoint was supposed to
        save. Pointing the wrapper at the reloaded models is what drops the
        second copy.
        ...
        *object*, so it would keep the discarded models alive on its own, and
        ...
        for sname, model in models.items():
            if sname in self.local_admm_stoch_subproblem_scenarios:
                self.local_admm_stoch_subproblem_scenarios[sname] = model
        self.varprob_dict = {}
