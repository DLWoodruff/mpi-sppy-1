...
class TestCheckpointBeforeSeconds(unittest.TestCase):
    ...
    def test_the_deadline_checkpoint_resumes(self):
        ...
        resumed = _make_ph(_options(total, resume_from=self.ckpt_dir))
        resumed.ph_main()
...
class TestIterationDurationIsRecorded(unittest.TestCase):
    ...
    def test_it_is_carried_across_a_resume(self):
        ...
        # Resuming without raising the limit runs no iterations at all, so
        # what is read here is the seed and nothing else.
        resumed = _make_ph(_options(2, resume_from=self.ckpt_dir))
        resumed.ph_main()
